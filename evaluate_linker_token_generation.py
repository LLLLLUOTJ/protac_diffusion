...
from sample_linker_token import (
    build_sampling_batch,
    choose_device,
    decode_results,
    load_token_diffusion,
    sample_tokens,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-evaluate token linker generation over multiple weak-anchor samples")
    ...
    parser.add_argument("--out-dir", type=str, default="outputs/linker_token_eval")
        ...
        out.append(
            {
                ...
                "source_oriented_token_smiles": items[0]["source_oriented_token_smiles"],
...
def plot_eval(per_source_rows: List[Dict[str, Any]], summary: Dict[str, Any], out_path: Path) -> None:
    ...
    axes[0].set_title(
        "Per-Source Token Generation Success "
...
def main() -> None:
    ...
    diffusion = load_token_diffusion(args.token_ckpt, device=device)
    ...
    for pos, dataset_index in enumerate(indices, start=1):
        ...
        rows = decode_results(
            dataset=dataset,
            batch=batch,
            sampled_token_x=sampled_token_x,
    ...
    print(f"[done] mode=token out_dir={out_dir}", flush=True)
    print(
